...
def main() -> int:
    ...
    receipt: dict[str, object] = {
        "checker": checker, "tool_version": version(distribution),
        "python": sys.version, "command": command, "git_sha": os.environ.get("GITHUB_SHA"),
        "lock_sha256": hashlib.sha256(Path("uv.lock").read_bytes()).hexdigest(),
        "scope": ["src", "scripts", "tests"], "baseline_suppression": False,
    }
    try:
        completed = subprocess.run(command, capture_output=True, text=True, check=False, timeout=600)  # noqa: S603 - closed-set argv and no shell
    ...
    (output / (checker + "-receipt.json")).write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
