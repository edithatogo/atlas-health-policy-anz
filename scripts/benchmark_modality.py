...
def main() -> int:
    ...
    parser.add_argument("--cases", default="quality/benchmarks/adversarial-modality-v1.jsonl")
    parser.add_argument("--output", default="build/benchmarks/modality-v1.json")
    args = parser.parse_args()
    metrics = evaluate_classifier(load_jsonl(args.cases), lambda text: classify_modality(text).modality)
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(metrics.as_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
