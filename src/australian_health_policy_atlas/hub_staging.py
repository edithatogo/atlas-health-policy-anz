...
class HfStore:
    ...
    def __init__(self, repo_id: str, token: str) -> None:
        if not token or repo_id != "edithatogo/au-health-policy-atlas-bronze":
            raise ValueError(
                "explicit Atlas Bronze target and write credential required"
            )
    ...
    def ensure_public(self) -> None:
        ...
        if info.private is not False:
            raise ValueError(
                "private dataset is not an allowed Atlas publication target"
            )
        if not info.sha:
            self.put({
...
def verify_stage(root: Path) -> dict[str, Any]:
    ...
    for item in inventory:
        ...
        if (
            not SHA256.fullmatch(item["sha256"])
...
def _remote_stage(
    hub: HubStore, reference: dict[str, Any], destination: Path
) -> dict[str, Any]:
    if destination.exists() and any(destination.iterdir()):
        raise ValueError("restore destination must be empty")
    verify_seal(reference)
    if not REVISION.fullmatch(reference["revision"]) or not SHA256.fullmatch(
...
def publish_stage(hub: HubStore, stage: Path) -> dict[str, Any]:
    ...
    files = {
        f"{prefix}/{item['path']}": safe_path(stage, item["path"])
        for item in manifest["files"]
    }
