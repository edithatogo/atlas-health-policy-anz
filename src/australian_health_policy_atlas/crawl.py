...
class CrawlPolicy:
    ...
    def validate(self) -> None:
        ...
        for n in (self.max_targets, self.max_links_per_page, self.max_attempts, self.max_bytes):
...
def check_url(url: str, hosts: tuple[str, ...]) -> None:
    ...
    if (parts.scheme != "https" or parts.hostname not in hosts or parts.username or
            parts.password or parts.port not in (None, 443) or parts.fragment or
...
def _new_target(url: str, depth: int, parent: str | None) -> dict[str, Any]:
    return {"url": url, "depth": depth, "parent": parent, "status": "queued", "attempts": 0}


def _fresh(policy: CrawlPolicy) -> dict[str, Any]:
    return {"schema_version": "1.0", "kind": "source-crawl-state",
            "policy": asdict(policy), "policy_sha256": sha256_json(asdict(policy)),
            "targets": [_new_target(policy.seed_url, 0, None)], "boundaries": [],
            "generation": 0}


def validate_state(state: dict[str, Any], policy: CrawlPolicy, root: Path) -> None:
    verify_seal(state)
    if state.get("kind") != "source-crawl-state" or state.get("policy_sha256") != sha256_json(asdict(policy)):
    ...
    for target in targets:
        ...
        if (type(target["depth"]) is not int or not 0 <= target["depth"] <= policy.max_depth or
                type(target["attempts"]) is not int or not 0 <= target["attempts"] <= policy.max_attempts):
        ...
        seen.add(url)
        if target["status"] not in {"queued", "retryable", "captured", "unavailable", "restricted", "oversized", "failed"}:
        ...
        if target["status"] == "captured":
            ...
            if obj.stat().st_size != receipt["size_bytes"] or sha256_file(obj) != receipt["sha256"]:
                raise ValueError("captured object fixity mismatch")


def crawl_readiness(state: dict[str, Any]) -> dict[str, Any]:
    counts = {key: sum(t["status"] == key for t in state["targets"]) for key in
              ("queued", "retryable", "captured", "unavailable", "restricted", "oversized", "failed")}
    exhausted = counts["queued"] + counts["retryable"] == 0
    return {"counts": counts, "frontier_exhausted": exhausted,
            "scope_complete": exhausted and not state["boundaries"] and counts["captured"] > 0 and
                not any(counts[k] for k in ("restricted", "oversized", "failed")),
            "state_sha256": state["sha256"], "state_generation": state["generation"],
            "not_corpus_completeness": True, "gate_b_passed": False}


def run_crawl(policy: CrawlPolicy, root: Path, *, request_budget: int = 20,
              fetch: Callable[..., CaptureReceipt] = capture_url) -> dict[str, Any]:
    policy.validate()
    ...
    state = read_json(state_path.read_bytes()) if state_path.exists() else sealed(_fresh(policy))
    ...
    while index < len(state["targets"]) and requests < request_budget:
        ...
        target["attempts"] += 1
        try:
            receipt = fetch(target["url"], cas_root=root / "cas", max_bytes=policy.max_bytes,
                            retries=0, allowed_hosts=policy.allowed_hosts)
            ...
            if receipt.media_type in {"text/html", "application/xhtml+xml"}:
                ...
                candidates = [l for l in links if l.likely_document or "page=" in l.url or "next" in l.anchor_text.lower()]
                ...
                for offset, link in enumerate(candidates):
                    ...
                    if reason:
                        ...
                        continue
                    state["targets"].append(_new_target(url, target["depth"] + 1, target["url"]))
                    known.add(url)
            ...
            if exc.code in {404, 410}:
                ...
                target["status"] = "retryable" if target["attempts"] < policy.max_attempts else "failed"
            else:
    ...
    atomic_json(state_path, state)
    validate_state(state, policy, root)
    return sealed({"schema_version": "1.0", "kind": "crawl-readiness", **crawl_readiness(state)})
