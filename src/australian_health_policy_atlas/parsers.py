...
def _parse_pdf_optional(path: Path, source_id: str) -> ParseResult:
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "PDF parsing requires a qualified optional parser: pymupdf or pypdf"
            ) from exc
        ...
        for page_number, block in enumerate(blocks, 1):
            for segment in normalize_text(
                f"{source_id}.p{page_number}", block, parser_id="pypdf-v1"
            ):
    ...
    for page_number, page in enumerate(document, 1):
        ...
        for segment in normalize_text(
            f"{source_id}.p{page_number}", block, parser_id="pymupdf-v1"
        ):
            segments.append(segment)
...
def _parse_docx_optional(path: Path, source_id: str) -> ParseResult:
    try:
        ...
        raise RuntimeError(
            "DOCX parsing requires the qualified optional parser python-docx"
        ) from exc
