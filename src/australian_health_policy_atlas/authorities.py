...
ROLES = frozenset({"professional_regulator", "education_accreditor", "accreditation_committee",
    "professional_standard_setter", "service_accreditor", "sector_regulator", "standard_setter",
    "rights_oversight", "policy_issuer", "funding_authority", "advisory_committee",
    "ethics_committee", "accreditation_body", "legislation_publisher"})
FIELDS = ("body_id", "name", "countries", "jurisdiction", "role", "url", "groups", "topics")
...
def source_bytes(name: str, directory: Path | None = None) -> bytes:
    ...
    return (Path(__file__).resolve().parents[2] / "data" / "sources" / name).read_bytes()


def load_authorities(directory: Path | None = None) -> list[dict[str, Any]]:
    reader = csv.DictReader(io.StringIO(source_bytes("authorities-anz-v1.csv", directory).decode("utf-8")))
...
def validate_authorities(records: list[dict[str, Any]]) -> None:
    ...
    for row in records:
        ...
        if not isinstance(countries, list) or not countries or len(set(countries)) != len(countries) or not set(countries) <= {"AU", "NZ"}:
            raise ValueError("invalid country scope")
        expected = "ANZ" if len(countries) == 2 else "NZ" if countries == ["NZ"] else None
        if (expected and row["jurisdiction"] != expected) or (countries == ["AU"] and row["jurisdiction"] in {"NZ", "ANZ"}):
            raise ValueError("country and source jurisdiction disagree")
        for key in ("groups", "topics"):
            values = row.get(key)
            if not isinstance(values, list) or not values or len(set(values)) != len(values) or any(not isinstance(v, str) or not IDENTIFIER.fullmatch(v) for v in values):
...
def coverage_report(records: list[dict[str, Any]], contract: dict[str, Any]) -> dict[str, Any]:
    """Report complete named directory sets separately from the open-world corpus."""
    validate_authorities(records)
    if type(contract.get("schema_version")) is not int or contract["schema_version"] != 1 or contract.get("open_world_complete") is not False:
    ...
    for group in contract["groups"]:
        ...
        if universe not in {"closed-directory-snapshot", "open"} or group.get("document_corpus_complete") is not False:
        ...
        if closed and (not required or not group.get("observed_on")):
            raise ValueError("closed directory requires observation date and explicit membership")
        if closed:
        ...
        reports.append({"group_id": identity, "registered": len(actual[identity]),
            "denominator": len(required) if closed else None, "denominator_unit": group["denominator_unit"],
            "missing": missing, "unexpected": unexpected,
            "status": "matched_directory_snapshot" if closed and not missing and not unexpected else "directory_mismatch" if closed else "open_scope",
            "evidence_url": group["evidence_url"], "document_corpus_complete": False})
    if set(actual) - known:
        raise ValueError("authority references undeclared coverage group")
    return sealed({"schema_version": 1, "kind": "authority-registration-coverage",
        "snapshot_id": contract["snapshot_id"], "registered_bodies": len(records),
        "groups": reports, "open_world_complete": False, "gate_b_passed": False,
...
def assert_directory_coverage(records: list[dict[str, Any]], contract: dict[str, Any]) -> dict[str, Any]:
    report = coverage_report(records, contract)
    if any(item["status"] == "directory_mismatch" for item in report["groups"]):
        raise ValueError("authority directory membership drift; update evidence, not just the count")
    return report


def acquisition_sources(collection: str = "authorities-v1", directory: Path | None = None) -> list[dict[str, Any]]:
    ...
    for (url, jurisdiction), bodies in sorted(grouped.items()):
        ...
        result.append({"source_id": "authority-" + ids[0], "url": url,
            "jurisdiction": jurisdiction, "body_ids": ids,
            "countries": sorted({c for b in bodies for c in b["countries"]}),
            "roles": sorted({b["role"] for b in bodies}), "registered_on": contract["registered_on"],
            "capture_status": "configured_unqualified", "bindingness": "not_inferred",
            "source_scope_not_document_applicability": True})
    return sorted(result, key=lambda x: x["source_id"])


def authority_policies(collection: str, directory: Path | None = None) -> list[CrawlPolicy]:
    policies = []
    for source in acquisition_sources(collection, directory):
        host = urlsplit(source["url"]).hostname
        policy = CrawlPolicy(source["source_id"], source["jurisdiction"], source["url"], (host,),
            source["registered_on"] + "T00:00:00Z", max_depth=2, max_targets=200,
            max_links_per_page=100, max_attempts=3, policy_version="bounded-authority-discovery-v1")
        policy.validate()
...
def authority_graph(directory: Path | None = None):
    ...
    from .graph import GraphNode, GraphEdge, PolicyGraph
    records = load_authorities(directory)
    ...
    for body in records:
        ...
        graph.add_edge(GraphEdge(identity, role, "REGISTERED_ROLE", {"document_bindingness": "not_inferred"}))
    ...
    for source in acquisition_sources("authorities-v1", directory):
        ...
        graph.add_node(GraphNode(identity, "source_registry_entry", source["url"], source))
        for body in source["body_ids"]:
            graph.add_edge(GraphEdge("authority:" + body, identity, "HAS_REGISTERED_SOURCE", {}))
    return graph
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = acquisition_sources(args.sources, args.directory) if args.sources else assert_directory_coverage(load_authorities(args.directory), load_contract(args.directory))
    if args.graph:
        graph = authority_graph(args.directory)
        result = {"kind": "authority-catalogue-projection", "nodes": [n.as_dict() for n in graph.nodes.values()], "edges": [e.as_dict() for e in graph.edges], "gate_b_passed": False}
