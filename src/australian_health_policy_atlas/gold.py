...
_MODALITY_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("must_not", re.compile(r"\b(?:must\s+not|shall\s+not|is\s+prohibited\s+from)\b", re.I)),
    ("must", re.compile(r"\b(?:must|shall|is\s+required\s+to|are\s+required\s+to)\b", re.I)),
    ("should", re.compile(r"\b(?:should|is\s+recommended\s+to|are\s+recommended\s+to)\b", re.I)),
    ("may", re.compile(r"\b(?:may|is\s+permitted\s+to|are\s+permitted\s+to)\b", re.I)),
)
...
def extract_timeframe(text: str) -> str | None:
    match = re.search(r"\bwithin\s+(\d+)\s+(minute|minutes|hour|hours|day|days|week|weeks)\b", text, re.I)
...
def extract_simple_assertion_fields(text: str) -> SimpleAssertionFields:
    ...
    if modality_result.modality is None:
        return SimpleAssertionFields(None, None, None, None, extract_timeframe(text), False, modality_result.reason_code)
    modal_phrases = {
    ...
    if not match:
        return SimpleAssertionFields(None, None, None, modality_result.modality, extract_timeframe(text), False, "complex_clause_requires_structured_extraction")
    obj = match.group("object").strip()
    timeframe = extract_timeframe(obj)
    if timeframe:
        obj = re.sub(r"\s+within\s+\d+\s+(?:minute|minutes|hour|hours|day|days|week|weeks)\b.*$", "", obj, flags=re.I).strip()
