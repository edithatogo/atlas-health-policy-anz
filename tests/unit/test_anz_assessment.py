...
spec = importlib.util.spec_from_file_location('assess_anz', ROOT / 'scripts/assess_bronze.py')
...
def test_distinct_scope_identities():
    ...
    assert au_kind.startswith('frozen') and nz_kind == anz_kind
...
def test_assessment_binds_selected_scope_without_network(monkeypatch, tmp_path, capsys):
    ...
    def assess(hub, policies, *, census_sha256, code_revision):
        assert len(policies) == 81 and code_revision == '1' * 40
        return sealed({'expected_sources': len(policies), 'census_sha256': census_sha256, 'gate_b_passed': False})
    monkeypatch.setattr(module, 'qualify_remote_bronze', assess)
    assert module.main(['--collection', 'nz-v1']) == 0
    row = json.loads(capsys.readouterr().out)
    verify_seal(row)
    assert row['collection'] == 'nz-v1' and row['gate_b_passed'] is False
