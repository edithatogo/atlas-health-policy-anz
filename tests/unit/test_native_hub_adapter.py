...
def test_native_hf_adapter_contract_without_network(monkeypatch,tmp_path):
    ...
    assert store.put({"data/object":path,"meta.json":b"{}"},parent="a"*40)=="b"*40
    with pytest.raises(ValueError):
        store.get("data/object","main")
    monkeypatch.setattr(hf,"hf_hub_download",lambda **_kw: (_ for _ in ()).throw(EntryNotFoundError("absent")))
