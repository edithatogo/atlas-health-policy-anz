...
@pytest.mark.parametrize("mutation",["policy","empty","duplicate","depth","attempts","root","status","receipt"])
def test_resealed_invalid_state_is_rejected(tmp_path,mutation):
    value=state(tmp_path)
    target=value["targets"][0]
    if mutation=="policy": value["policy"]["cutoff"]="changed"
    if mutation=="empty": value["targets"]=[]
    if mutation=="duplicate": value["targets"].append(dict(target))
    if mutation=="depth": target["depth"]=-1
    if mutation=="attempts": target["attempts"]=-1
    if mutation=="root": target["parent"]="bogus"
    if mutation=="status": target["status"]="made-up"
    if mutation=="receipt": target["receipt"]["requested_url"]="https://health.test/wrong"
    with pytest.raises(ValueError): validate_state(sealed(value),policy(),tmp_path)


def test_bad_parent_chain(tmp_path):
    value=state(tmp_path)
    target=value["targets"][0]
    target.update(url="https://health.test/child",depth=1,parent="not-a-parent",status="queued")
...
def test_capture_contract_failures_do_not_become_evidence(tmp_path,mutation):
    ...
    def invalid(url,**kwargs):
        ...
        if mutation=="url": return replace(receipt,requested_url="https://health.test/wrong")
        if mutation=="outside": return replace(receipt,stored_path=str(tmp_path.parent/"escape"))
...
def test_cycle_and_duplicate_links_terminate(tmp_path):
    pages={policy().seed_url:b'<a href="/policies">Policies</a><a href="/a.html">Policy</a>',
