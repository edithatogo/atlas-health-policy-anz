...
from australian_health_policy_atlas.integrity import read_json, sealed, atomic_json


def policy(**changes):
    base = CrawlPolicy("qld-test", "QLD", "https://health.test/policies", ("health.test",), "2026-09-03")
...
def fetcher(pages, seen=None):
    def fetch(url, *, cas_root, **_kwargs):
        ...
        return CaptureReceipt(url, url, 200, "2026-09-05T00:00:00+00:00", digest, len(data),
                              "application/pdf" if url.endswith(".pdf") else "text/html", None, None, str(file))
    return fetch


def test_crawl_resume_without_recapture(tmp_path):
    pages = {"https://health.test/policies": b'<a href="/policy.pdf">Policy</a>',
    ...
    assert second["scope_complete"] and not second["gate_b_passed"]
    assert second["counts"]["captured"] == 2
    run_crawl(policy(), tmp_path, fetch=fetcher(pages, seen))
    assert len(seen) == 2


@pytest.mark.parametrize("changes,reason", [({"max_depth": 0}, "depth_limit"),
    ({"max_targets": 1}, "target_limit"), ({"max_links_per_page": 1}, "link_limit")])
def test_limits_are_not_exhaustive_coverage(tmp_path, changes, reason):
    pages = {"https://health.test/policies": b'<a href="/a.pdf">A</a><a href="/b.pdf">B</a>',
             "https://health.test/a.pdf": b"a", "https://health.test/b.pdf": b"b"}
    ...
    assert reason in {b["reason"] for b in read_json((tmp_path / "state.json").read_bytes())["boundaries"]}


def test_external_link_is_accounted(tmp_path):
    result = run_crawl(policy(), tmp_path, fetch=fetcher({"https://health.test/policies":
        b'<a href="https://other.test/policy.pdf">Policy</a>'}))
    assert not result["scope_complete"]


@pytest.mark.parametrize("status,expected", [(404,"unavailable"),(410,"unavailable"),(403,"restricted"),
    (401,"restricted"),(400,"failed"),(429,"retryable"),(503,"retryable")])
def test_http_dispositions(tmp_path, status, expected):
    result = run_crawl(policy(), tmp_path, fetch=fetcher({"https://health.test/policies":
        HTTPError("https://health.test/policies", status, "test", {}, None)}))
...
@pytest.mark.parametrize("error,kind", [(ValueError("max_bytes exceeded"),"oversized"),
    (ValueError("bad capture"),"failed"),(OSError("temporary"),"retryable")])
def test_capture_errors(tmp_path, error, kind):
    result = run_crawl(policy(), tmp_path, fetch=fetcher({"https://health.test/policies": error}))
...
@pytest.mark.parametrize("changes", [{"max_targets":0},{"max_attempts":True},{"max_depth":-1},
    {"source_id":"../x"},{"jurisdiction":"XX"},{"cutoff":""},{"allowed_hosts":("wrong.test",)}])
