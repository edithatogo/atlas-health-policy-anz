...
def test_hf_candidate_replays_bronze_bytes(tmp_path: Path) -> None:
    ...
    obj = ingest_local_file(source, source_id="p", source_uri="https://example.test/p", cas_root=tmp_path / "cas", observed_at="2026-09-03T00:00:00+00:00")
    manifest = write_manifest([obj], tmp_path / "manifest.json", release_id="b1")
    receipt = build_bronze_hf_candidate(manifest, output_dir=tmp_path / "hf", dataset_id="edithatogo/test")
