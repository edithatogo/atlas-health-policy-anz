...
from australian_health_policy_atlas.integrity import atomic_json, read_json, safe_path, sealed, verify_seal
...
@pytest.mark.parametrize("data", [b'{"a":1,"a":2}', b'[]', b'{"x":NaN}', b'{"x":Infinity}'])
def test_strict_json(data):
    with pytest.raises(ValueError):
        read_json(data)


@pytest.mark.parametrize("path", ["../x", "/x", "a/../../x", "C:/x", "a\\x", "", "a//x"])
...
@pytest.mark.parametrize("acceptance", [{}, {"fixity": "false"}, {"fixity": 1}, {"fixity": []}, {"fixity": None}])
def test_empty_and_non_boolean_acceptance_blocked(acceptance):
    assert not promotion_gate(MedallionLayer.CENSUS, closed_layers=set(), acceptance_results=acceptance).permitted
