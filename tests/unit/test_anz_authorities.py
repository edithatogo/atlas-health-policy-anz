...
def test_independent_closed_directory_denominators():
    ...
    verify_seal(report)
    expected = {
        "au-national-boards": 15,
        "nz-hpca-authorities": 18,
        "au-external-accreditors": 10,
        "au-accreditation-committees": 6,
        "au-specialist-colleges": 16,
        "nsw-professional-councils": 15,
        "au-health-complaints": 8,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("role", "government-means-binding"),
        ("url", "http://example.org/"),
        ("url", "https://u:p@example.org/"),
        ("url", "https://example.org/#fragment"),
        ("url", "https://example.org:4433/"),
        ("url", None),
        ("url", "no-host"),
        ("groups", []),
        ("groups", ["x", "x"]),
...
def test_shared_portal_deduplicates_capture_not_bodies():
    ...
    assert len(councils[0]["body_ids"]) == 15
...
def test_graph_does_not_forge_legal_edges():
    ...
    assert "authority:nz-medsafe" in g.nodes
    assert "authority:nz-pharmac" in g.nodes
...
def test_nz_nlp_exact_offsets_and_no_fake_independence():
    ...
    assert {"New Zealand", "Ngā Paerewa", "Te Tiriti o Waitangi"} <= {
