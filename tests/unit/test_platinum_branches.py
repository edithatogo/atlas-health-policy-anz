...
def test_candidate_equivalent_and_not_equivalent() -> None:
    relationship, state, _ = baseline_relationship(
        "a b c", "a b c", left_modality="must", right_modality="must", threshold=0.5
    )
    assert relationship == "candidate_equivalent"
    assert state is EvidenceState.SUPPORTED_NEEDS_VERIFICATION
    relationship2, _, _ = baseline_relationship(
        "a", "z", left_modality=None, right_modality=None, threshold=0.5
    )
