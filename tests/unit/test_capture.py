...
class FakeResponse:
    def __init__(self, data: bytes = b"abc", url: str = "https://health.test/final", status: int = 200) -> None:
        self._data = data
        self._url = url
        self.status = status
...
def test_capture_url_writes_cas_and_receipt(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_capture_rejects_oversized_payload(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(capture, "urlopen", lambda *_args, **_kwargs: FakeResponse(b"abcd"))
    with pytest.raises(ValueError, match="max_bytes"):
        capture.capture_url("https://health.test/x", cas_root=tmp_path, max_bytes=3, retries=0)


def test_capture_retries_then_succeeds(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_capture_raises_last_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(capture, "urlopen", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("dead")))
