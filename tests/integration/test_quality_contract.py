...
def test_strict_configuration_is_executable_contract() -> None:
    ...
    config = cast("dict[str, object]", tomllib.loads((root / "pyproject.toml").read_text(encoding="utf-8")))
...
def test_external_tool_contract_uses_autospec(mocker: MockerFixture) -> None:
    ...
    patched = mocker.patch("australian_health_policy_atlas.external_tools.subprocess.run",
                           autospec=True, return_value=completed)
...
def test_http_contract_uses_loopback_only(httpserver: HTTPServer) -> None:
    ...
    with urlopen(httpserver.url_for("/manifest"), timeout=2) as response:  # noqa: S310 - test server is bound to loopback, pytest-socket restricts connections
        assert response.read() == b"synthetic-capture"


@pytest.mark.quality
def test_inline_snapshot_freezes_rejection_reasons() -> None:
    decision = promotion_gate(MedallionLayer.SILVER, closed_layers=set(), acceptance_results={})
    assert {"permitted": decision.permitted, "reasons": list(decision.reasons)} == snapshot({
        "permitted": False,
