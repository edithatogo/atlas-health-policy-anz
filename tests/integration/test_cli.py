...
def test_bronze_cli(tmp_path: Path) -> None:
    ...
    value = record(decode_json(manifest.read_text(encoding="utf-8")))
...
def test_cli_nlp_and_graph_query(
    ...
    nlp_payload = record(decode_json(capsys.readouterr().out))
    ...
    payload = record(decode_json(capsys.readouterr().out))
    assert payload["hits"]
